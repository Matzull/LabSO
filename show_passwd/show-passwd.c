...
passwd_entry_t* parse_passwd(struct options* options, int* nr_entries)
{
	...
	int cur_line;
	// "/etc/passwd"
	if ((passwd=fopen(options->inPath,"r"))==NULL) {
		fprintf(stderr, options->inPath, " could not be opened: ");
		perror(NULL);
		return NULL;
	}
...
int main(int argc, char *argv[])
{
	...
	//implement a new -c option in the program, which allows displaying the fields in each passwd entry as 
	//comma-separated values (CSV) instead of ":".

	/* Initialize default values for options */
	options.outfile = stdout;
	options.output_mode = VERBOSE_MODE;
	options.inPath = clone_string("/etc/passwd");
	...
	while((opt = getopt(argc, argv, "hvpo:i:c")) != -1) {
		...
		case 'o':
			...
			break;
		case 'i':
			options.inPath=clone_string(optarg);
			break;
		case 'c':
